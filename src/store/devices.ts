...
export class DeviceStore {
  ...
  handleEVENT_UPDATE_CONTROLLER_DATA(original_s: string): void {
    ...
    const userdata_area: string = JSON.stringify(
      jsonObject[KEYS.USERDATA_AREA]
    );
    ...
    const system_list: { [key: string]: number[] } = JSON.parse(
      jsonObject[KEYS.SYSTEM_LIST]
    );
    const belong_groups: string[] = JSON.parse(jsonObject[KEYS.BELONG_GROUPS]);
